...
function Blog({ blog ,handleEdit, handleDelete,handleView}) {
    const { _id, title, likes, image, category, createdAt, comments, readmin } = blog;
...
function Blogsmanagement() {
    ...
    return (
        <div className='Blogs list-of-items-container'>
            <h1>List of Blogs</h1>
            <button className='add-button' onClick={handleAddSection}>Add blog</button>
            
            {isFormOpen && <BlogForm handleClose={handleCloseForm} BlogToUpdate={blogToUpdate} handleReload={handleReload} /> }
            ...
            {modal && <BlogModalContent handleClose={handleCloseModal} props={selectedBlog}/>}
            <div className='pagination'>
                <button onClick={() => paginate(currentPage - 1)} disabled={currentPage === 1}>
                    Previous
                </button>
                <button onClick={() => paginate(currentPage + 1)} disabled={indexOfLastItem >= blogs.length}>
                    Next
                </button>
